    ...
    with st.spinner("Searching for answers..."):
        if uploaded_file:
            ...
            if is_general_heal_query:
                ...
                # Enhanced protocol analysis prompt
                protocol_prompt = f"""You are an advanced AI designed to analyze clinical research protocols for the HEAL Research Dissemination Center.

                When analyzing protocols and answering questions:
                1. Focus on extracting structured information using exact wording when possible
                2. Identify and categorize key protocol elements including:
                   - Study design and methodology
                   - Eligibility criteria
                   - Interventions and treatments
                   - Endpoints and outcomes
                   - Timelines and milestones
                3. Note any missing or unclear information explicitly
                4. Use standardized terminology where appropriate
                5. If information isn't in the provided sections, say so clearly
                ...
                If the question requires structured data extraction, use this JSON schema:
                {{
                    "study_title": "",
                    "clinical_trial_id": "",
                    "study_design": {{
                ...
                Answer based ONLY on the protocol sections provided, maintaining accuracy and precision:"""

            # Use appropriate prompt based on query type
            response = openai_client.chat.completions.create(
                model=OPENAI_MODEL,
                messages=[{"role": "user", "content": general_prompt if is_general_heal_query else protocol_prompt}],
                temperature=0.7,
                max_tokens=None
            )
